...
let RATE_THBC_PER_USDT = 35; // default; จะอัปเดตจาก contract หลังเชื่อม
...
async function connectWallet() {
  try {
    if (!window.ethereum) {
      alert("Please install MetaMask, Bitget or a Web3 wallet.");
      return;
    }
...
// ✅ เวอร์ชันใหม่: Approve แบบ MAX_UINT (Unlimited) สำหรับ Bitget + ทุก wallet
async function approveUSDT() {
  ...
  try {
    setStatus(
      currentLang === "th"
        ? "กำลังอนุมัติ USDT แบบไม่จำกัด..."
        : "Sending unlimited USDT approval...",
      ""
    );

    // ถ้ามี allowance อยู่แล้วเยอะพอ ให้แจ้งแล้วไม่ต้องอนุมัติซ้ำก็ได้
    const currentAllowance = await usdtContract.methods
      .allowance(selectedAccount, THBC_CONFIG.EXCHANGE_ADDRESS)
      .call();

    const needReapprove = web3.utils
      .toBN(currentAllowance)
      .lt(web3.utils.toBN("1000000000000000000")); // < 1 USDT ถือว่าน้อยไป

    if (!needReapprove) {
      setStatus(
        currentLang === "th"
          ? "มีการอนุมัติ USDT อยู่แล้ว"
          : "USDT already approved.",
        "success"
      );
      return;
    }
    ...
    await usdtContract.methods
      .approve(THBC_CONFIG.EXCHANGE_ADDRESS, MAX_UINT)
      .send({ from: selectedAccount });

    setStatus(
      currentLang === "th"
        ? "อนุมัติ USDT แบบไม่จำกัดสำเร็จ"
        : "Unlimited USDT approval successful.",
      "success"
    );
  } catch (err) {
    console.error(err);
...
async function buyTHBC() {
  ...
  try {
    ...
    if (
      ...
      refToUse = null; // ป้องกัน self-ref
    }
